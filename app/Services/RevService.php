...
class RevService
{
    ...
    public function getProductsList($filters = [])
    {
        ...
        try {
            ...
            // Extract ID, Name or Barcode filter if present
            $idFilter = '';
            $nameFilter = '';
            $barcodeFilter = '';
            ...
            // Add ID, Barcode or Name to URL path if specified
            if (!empty($idFilter)) {
                $url .= 'ID/' . urlencode($idFilter) . '/';
            } elseif (!empty($barcodeFilter)) {
    ...
    /**
     * Get records from a generic ERPRev endpoint with path-based pagination
     */
    public function getEndpointData($endpoint, $filters = [])
    {
        if (!$this->enabled) {
            return ['success' => false, 'message' => 'ERPREV sync is disabled'];
        }
        ...
        try {
            // Trim leading/trailing slashes from endpoint
            $endpoint = trim($endpoint, '/');
            
            // Base URL with endpoint
            $url = $this->baseUrl . '/' . $endpoint . '/json';
            
            // Handle lastupdated (defaults to 'all' if not specified)
            $lastUpdated = $filters['lastupdated'] ?? 'all';
            if (empty($lastUpdated)) {
                $lastUpdated = 'all';
            }
            
            // Parse relative string (e.g. '100d', '5m') to a real date string
            $lastUpdated = $this->parseLastUpdated($lastUpdated);
            
            $url .= '/lastupdated/' . rawurlencode($lastUpdated);
            
            // Handle pagination startRow and TotalRecords if provided
            if (isset($filters['startRow']) && $filters['startRow'] !== '') {
                $url .= '/startRow/' . urlencode($filters['startRow']);
            }
            if (isset($filters['TotalRecords']) && $filters['TotalRecords'] !== '') {
                $url .= '/TotalRecords/' . urlencode($filters['TotalRecords']);
            }
            
            // Collect any other filters to send as query parameters
            $queryParams = array_diff_key($filters, array_flip(['lastupdated', 'startRow', 'TotalRecords']));
            
            Log::info("ERPREV Service - getEndpointData - Making API call", [
                'endpoint' => $endpoint,
                'url' => $url,
            ...
            $response = Http::withHeaders([
                'Authorization' => $this->getAuthHeader(),
                'Accept' => 'application/json',
            ])->timeout(120)->get($url, $queryParams);

            if ($response->successful()) {
                $data = $response->json();
                
                $this->logSync('sales', 'success', "Fetched data from {$endpoint}", [
                    'count' => count($data['data'] ?? $data['records'] ?? $data['records_view'] ?? []),
                    'url' => $url,
                ]);
            ...
            $this->logSync('sales', 'error', "Error fetching {$endpoint}: " . $e->getMessage());
            return ['success' => false, 'message' => $e->getMessage()];
        }
    }

    /**
     * Parse relative lastupdated filter to date string if needed
     */
    private function parseLastUpdated($value)
    {
        ...
        if (preg_match('/^(\d+)([mhdy])$/i', $value, $matches)) {
            ...
            // For days/years filter, use Y-m-d format to keep it simple and safe for URL paths
            if ($unit === 'd' || $unit === 'y') {
    ...
    {
        return $this->getEndpointData('rendered-services-view', $filters);
    }

    public function getQuotations($filters = [])
    {
        return $this->getEndpointData('get-quotations', $filters);
    }

    public function getQuotationProductsView($filters = [])
    {
        return $this->getEndpointData('quotation-products-view', $filters);
    }

    public function getQuotationServicesView($filters = [])
    {
        return $this->getEndpointData('quotation-services-view', $filters);
    }
}